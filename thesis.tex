...
parallel; if one these versions fail, the mechanism transparently switches to
...
suited for executing multiple different versions in parallel.

In this thesis, we present two different designs for building such monitors;
one, called \mx is focused on surviving crashes caused by bugs introduced in
software updates and a second, called \varan is aimed at running a large number
of versions side-by-side with low overhead.
