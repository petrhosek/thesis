...
\dedication{I would like to dedicate this thesis to everyone who believed in me when others did not. You have my undying gratitude.} 
\submitdate{31 March 2015}
...
With the widespread availability of multi-core processors, multi-version
execution might become a viable approach for increasing the reliability and
availability of updated software systems.

In this thesis, we propose two different schemes for implementing the
...
The key component of such $N$-version execution (NVX) systems is a runtime
monitor that enables the execution of multiple versions in parallel.
...
acts as a performance bottleneck. Finally, existing monitors can only run
multiple semantically-equivalent variants in parallel, and not multiple
software versions.

In this thesis, we present two different designs for building such monitors;
the first, called \mx is focused on surviving crashes caused by bugs introduced
in software updates and the second, called \varan is aimed at running a large
number of versions side-by-side with low overhead.
...
\lighttpdgen, a popular web server used by several high-traffic websites; and
