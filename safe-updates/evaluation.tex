...
The bug affecting both \mdsum and \shasum utilities introduced in 5.1
...
\textstt{issetugid()} (\S\ref{sec:applications}).  The upper bound for
\lighttpd issue \#2169 adds a \textstt{read} call to
\textstt{/dev/[u]random}, in order to provide a better entropy source
for generating HTTP cookies.  This additional \textstt{read} call
...
\paragraph{\redis} To measure the performance overhead for \redis, we used the
