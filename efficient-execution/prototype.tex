...
to start them (Step \circl{A} in Figure~\ref{fig:setup}). \nx is built
...
%The coordinator is linked with the \nx library (\nxlib, stored in
%\stt{libvaran.so}), which will be also injected inside each version.
%\nxlib is built as a statically-linked, position-independent library,
...
%% Afterwards, \nx attaches the shared memory segment and initializes
...
%% \nx is operates at the thin boundary between the operating system and
...
%% approaches. \nx does not require the use of dynamic linker, as
%% required by \stt{LD\_PRELOAD}. 


\boldtext{Monitor.} To ensure that \nx can be compiled as
...
%% \nxlib provides its own entry point (\ie the \stt{\_start} function),
%% which is the starting point for \nx's execution. After start, it first
...
%% address space. Since \nx operates as a pre-loader, it is responsible
...
%% \nx loads the linker image specified in the program header as
...
the call.
%% \nx allows both the system call table and the default system call
%% handler to be provided by the embedder of \nx \todo{Where shall we
%%   described the possibility of \nx embedding, \ie building tools on
%%   top of \nx?}. This makes it easy to customize \nx behavior for
...
%% generator. We have also used the generator throughout the \nx
...
%In \nx, binary rewriting is also used to intercept virtual system
%calls.
...
As in any NVX system operating at the level of system calls, \nx has
...
%% All application instances running in parallel under \nx are assigned
...
% \nx recognizes several different types of events:
% \begin{inparaenum}[(i)]
...
Since one of our primary goals for \nx was to minimize the performance
...
%have to be terminated.  We first note that \nx has not detected any
...
%% performance of \nx, and this is something we would like to address in
%% our future work.
