...
allow the use of Linux \textstt{perf}
...
\textstt{perf} has some advantages compared to other similar tools such as
\textstt{strace}\footnote{\url{http://sourceforge.net/projects/strace/}}, most
notably the performance overhead, which is significantly lower in case of
\textstt{perf} as the tool is implemented largely in kernel while
\textstt{strace} uses the \ptrace interface. This minimizes the potential
...
for I/O intensive applications such as \redis, as the \textstt{perf} tool uses
a custom binary format which contains large number of additional information
compared to the simple textual output of \textstt{strace}.
...
After the recording, we used the \textstt{perf} tool to extract the system call
...
\textstt{000-00.trace}, the first thread spawned by the main thread would be
stored in the file \textstt{000-01.trace}, the main thread of the second
process would use the file \textstt{001-00.trace}, \etc
...
We then compared the post-processed traces of the consecutive revisions using
...
These results show that in changes in externally observable behaviour are
sporadic. In particular, the revisions which introduced bugs described in
Section~\ref{multi-version:scenarios} are among revisions which did not
introduce any changes, yet these revisions are responsible for critical crash
bugs.
...
the range 2379--2635 of branch \textstt{lighttpd-1.4.x}, which were developed
