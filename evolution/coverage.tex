...
terms of \si{\eloc}.  As discussed above, we measured the number of \si{\eloc} in
...
reasons for which we have decided to measure \si{\eloc} rather than other
...
with periods of intense development that increase the \si{\eloc}
...
note that there are also several revisions where the number of \si{\eloc}
...
%% beginning of the period analysed, with higher \si{\eloc} churn than during
...
The total number of \si{\eloc} added or modified varies
between \redisPatchTotal for \redis and \lighttpdtwoPatchTotal
for \lighttpdtwo, while the end-to-end difference in \si{\eloc} varies
...
\beanstalkdNoTestNoExecutableRevs in \beanstalkd, \gitNoTestNoExecutableRevs in
\git and \vimNoTestNoExecutableRevs in \vim. Note that some of these patches
may modify code that is excluded during preprocessing on our machine, but most
cases involved changes to build scripts, documentation, and other similar
software artefacts.
...
useful guidance for the external behaviour changes---small, localised changes
...
executable code patches analysed in each system, measured in \si{\eloc}. Note that
...
the median number of \si{\eloc} ranging from \vimPatchMedian to \gitPatchMedian.
...
patches are small and localised suggests that individual patches are likely to
...
\begin{tabular}{lrrrr}
\toprule
\multicolumn{1}{c}{}  & \textsc{Nondet.} & \multicolumn{3}{c}{\sc Nondet. \si{\eloc}} \\ 
\cmidrule{3-5}
...
reported success while another reported failure.  We have found a number of
revisions displaying this behaviour, as listed in Table~\ref{tbl:nondet} under
the column \textit{Nondet Result}.
...
\binutils, \lighttpd and \vim seems to be deterministic with respect
...
line as covered if the test suite {\em may} execute it.  The darker grey
...
\begin{table}[t]
\centering
\caption{Overall patch coverage bucketed by the size of the patch in \si{\eloc}. \textsc{NP} is the number of patches in the bucket and \textsc{C} is their overall coverage.  Only patches which add or modify executable code are considered.}
...
Table~\ref{tbl:patch-coverage-buckets} presents the same patch coverage
statistics, but with the patches bucketed by their size into three categories:
less than \SI{10}{\eloc}, between \SIrange{11}{100}{\eloc}, and greater than
\SI{100}{\eloc}.  For all benchmarks, patches are distributed similarly across
buckets, with the majority of patches having $\le$\SI{10}{\eloc} and only a few
exceeding \SI{100}{\eloc}. Except for \beanstalkd and \vim, the average
coverage of patches with $\le$\SI{10}{\eloc} is higher than for those with
\textgreater\SI{100}{\eloc}, but the coverage of the middle-size category
varies.

Finally, the first column in Table~\ref{tbl:latent} shows the overall
patch coverage, \ie the percentage of covered \si{\eloc} across all patches.  For
...
recorded the number of \si{\eloc} in each patch which are only covered in
the next few revisions (we considered up to ten subsequent revisions).
We refer to the ratio between the number of such \si{\eloc} and the total
patch \si{\eloc} as \textit{latent patch coverage}.
...
in \lighttpdtwo, where 12 revisions which only add tests cover an additional
\SI{74}{\eloc}) and patch code being enabled later on. In fact, the majority of
latent patch coverage in \lighttpdtwo---337 lines---is obtained by 6 revisions
which change no test files.  Upon manual inspection, we found that the code
...
multi-version execution can be useful in those scenarios.
